...
import random
import jieba
from sklearn.naive_bayes import MultinomialNB  # 使用多项式模型生成贝叶斯分类器
...
def MakeWordsSet(words_file):
    ...
    """
    words_set = set()
    with open(words_file, 'r', encoding='utf8') as fp:
...
def TextProcessing(folder_path, test_size=0.2):
    ...
    Parameters:
        folder_path - 文本存放的路径
        test_size - 测试集占比，默认占所有数据集的百分之20（五分之一）
    Returns:
    ...
    for folder in folder_list:
        ...
        for file in files:
            if j > 100:  # 每类text样本数最多100
                break
            with open(os.path.join(new_folder_path, file), 'r', encoding='utf8') as fp:
                raw = fp.read()
...
if __name__ == '__main__':
    ...
    # 文本特征提取和分类
    deleteN = 480  # 原始数据为450
    feature_words = words_dict(all_words_list, deleteN, stopwords_set)
    train_feature_list, test_feature_list = TextFeatures(train_data_list, test_data_list, feature_words)
    print(TextClassifier(train_feature_list, test_feature_list, train_class_list, test_class_list))
